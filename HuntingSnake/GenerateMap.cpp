...
void create_obstacle_2(unsigned int x_pos, unsigned int y_pos, unsigned int height, unsigned int width, Point obs[], int& obs_nums) {
	int dist = 20; // dist la gia tri khoang cach tu hai bien trai va phai den vi tri dat thanh vat can ngang
	for (int i = x_pos + dist; i <= width - dist; i++) { // vat can cach bien trai va bien phai 1 khoang = dist
		...
		obs[obs_nums].x = i;
		obs[obs_nums].y = y_pos + 10;
		obs_nums++;

		// Thanh vat can phia duoi
		obs[obs_nums].x = i;
		obs[obs_nums].y = y_pos + height - 10;
		obs_nums++;
	}
}
//draw_matchBoard(0, 8, 45, 110, SCORE, LEVEL);
// Generate level 2
void play_match2(unsigned int x_pos, unsigned int y_pos, unsigned int height, unsigned int width, Point obs[], int& obs_nums, int &SCORE, int &LEVEL) {
	...
	// draw match board
	draw_matchBoard(x_pos, y_pos, height, width, SCORE, LEVEL);
...
void create_obstacle_3(unsigned int x_pos, unsigned int y_pos, unsigned int height, unsigned int width, Point obs[], int& obs_nums) {
	...
	for (int j = y_pos + 15; j <= y_pos + height - 15; j++) {
		...
		obs[obs_nums].y = j;
		obs_nums++;
		obs[obs_nums].x = x_pos + 2 * dist + 1; // vat can dung cach 2 bien mot gia tri la 2*dist
		obs[obs_nums].y = j;
		obs_nums++;
